...
# file = open(r"/data/zengyanru/DeepSumo/PretreatData/Result/blosum62.txt", "r")
file = file.readlines()
...
def get_rate(_line, _positive_line, _negative_line, avg2):
    ...
    for positive_line in _positive_line:
        positive_line = positive_line.strip()[30 - int(avg2):31 + int(avg2)]
    ...
    for negative_line in _negative_line:
        negative_line = negative_line.strip()[30 - int(avg2):31 + int(avg2)]
    ...
    positive_rate_list = []
    for i in range(30):  # 改
        p = (i + 1) / 100  # used to be 1/100, but it will lead to zero division if sample size are less than 100
        np_sum = int(len(_positive_line) * p)
        # np_sum = np.ceil(len(_positive_line) * p)
...
def KNN_code(aa_line, _positive_line, _negative_line, avg2):
    ...
    for line in aa_line:
        line = line.strip()
        line = line[30 - int(avg2):31 + int(avg2)]
        data_list.append(
