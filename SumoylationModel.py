...
class CNN_SUMO:
    def __init__(self, all_id=None ,input_data=None,input_target=None,valid_data=None,valid_target=None,keep_prob=0.9,
                 bp_num=1, model_mode='train',mode='onehot',batch_size=20,epochs=100,learning_rate=0.01,
                 model_path=None,model_name=None,display_step=10,early_stop_thresh=0.000001,verbose=True,train_length=7):
        ...
        # in case of an input of unbalanced training dataset
    ...
    def conf_model(self):
        # onehot encode. Make this matrix to be graphs of corresponding sequences
        ...
        # onehot
        hidden_conv1 = tf.layers.conv2d(inputs=x_matrix,filters=8,kernel_size=[21,4],strides=[self.bp_num*21,1],padding="same",
            ...
            if epochs % self.display_step == 0:
                ...
                if self.verbose is True:
                    if self.valid_data is not None and self.valid_target is not None:
                        # vd1,vd3 = self.valid_data[:,0:self.dim],self.valid_data[:,self.dim:(self.dim+self.dim3)]
                        vd1,vd2 = self.valid_data[:, 0:self.dim],self.valid_data[:,self.dim:(self.dim+self.dim2)]
                        pred = sess.run(model,feed_dict={self.xs:vd1,self.xs2:vd2,self.keep_prob:self.keep_prob_rate})
                        # pred = sess.run(model, feed_dict={self.xs:vd1,self.xs3:vd3, self.keep_prob: self.keep_prob_rate})
    ...
    def predict(self,other_data=None,model_path=None,modelname=None):
        ...
        with tf.Session() as sess:
            ...
            pred = sess.run([model_out], feed_dict={xs:onehot_data,xs2:knn_data,
                                                    keep_prob:self.keep_prob_rate})
